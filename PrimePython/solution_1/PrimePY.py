...
class prime_sieve(object):
    ...
    def GetBit(self, index):
        ...
        return False if (index % 2 == 0) else self.rawbits[int(index/2)]
    ...
    def printResults(self, showResults, duration, passes):
        ...
        for num in range(3, self.sieveSize): # Count (and optionally dump) the primes that were found below the limit
            if self.GetBit(num) == True:
                if showResults:
                    stdout.write(f"{str(num)}, ")
                count+=1

        assert count == self.countPrimes()
        stdout.write("\n");
        print(
            f"Passes: {str(passes)}, Time: {str(duration)}, Avg: {str(duration / passes)}, Limit: {str(self.sieveSize)}, Count: {str(count)}, Valid: {str(self.validateResults())}"
        )

        # Following 2 lines added by rbergen to conform to drag race output format
        stdout.write("\n");
        print(f"davepl;{str(passes)};{str(duration)};1;algorithm=base,faithful=yes");
